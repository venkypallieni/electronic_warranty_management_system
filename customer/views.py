    ...
    if request.method == 'POST':
        try:
            ...
            if user_form.is_valid() and customer_form.is_valid():
                ...
                token_resp = s3_helper.get_token(request, S3_LAMBDA_GATEWAY,'customers', str(profile_photo))
                if(token_resp['status']=='OK') :
                    print('upload document respone: ', s3_helper.upload(token_resp['data'], profile_photo))
                customer.image_src = f"{token_resp['data']['url']}{token_resp['data']['fields']['key']}"
                user.save()
                customer.user = user
                customer.save()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                token_resp = s3_helper.get_token(request, S3_LAMBDA_GATEWAY, 'products', str(purchase_document))
                if(token_resp['status']=='OK') :
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                token_resp = s3_helper.get_token(request, S3_LAMBDA_GATEWAY, "warranties", str(warranty_document))
                if(token_resp['status']=='OK') :
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                prev_claims = data_helper.get_all_claims(WMS_MODALS, customer)
                validate_claim_warranty(warranties)
                print('warranties: ', warranties)
                claim_document  = request.FILES['repair_document']
                token_resp = s3_helper.get_token(request, S3_LAMBDA_GATEWAY, "claims", str(claim_document))
                if(token_resp['status']=='OK') :
                    print('upload document respone: ', s3_helper.upload(token_resp['data'], claim_document))
                new_claim.document_url = f"{token_resp['data']['url']}{token_resp['data']['fields']['key']}"
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if max_claim_amount < extended_warranty.warranty_amount:
                    raise WarrantyException('Calculated MaxWarrantyAmount eligible :', max_claim_amount)
